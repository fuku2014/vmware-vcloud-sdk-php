...
/**
 * This samples can be used by System Administrator to query the Admin AuditEvents using query.
 */
...
$longs  = array(
    ...
    "certpath:",    //-a|--certpath    [optional] local certificate path
);
...
if ($flag==true)
{
    ...
    // Display some of the Admin AuditEvent's information.
    displayAdminAuditEvents();
}
else
{
 ...
 */
function displayAdminAuditEvents()
{
    global $service;
    echo "Querying Admin AuditEvent's\n";
    // admin event query
    $query = $service->getQueryService();
    $records = $query->queryRecords(VMware_VCloud_SDK_Query_Types::ADMIN_EVENT);
    if (!empty($records))
    {
        echo "Some of the Admin AuditEvents :\n";
        $records = $records->getRecord();
        // iterate through the admin event's
        foreach ($records as $record)
        {
            $sdkAuditEvent = $service->createSDKObj($record->get_href());
            $auditEvent = $sdkAuditEvent->getAuditEvent();
            echo "--------------------------------------------------------------------------------------\n";
            echo "Event ID: " . $auditEvent->get_id()."\n";
            echo "Event Type: " . $auditEvent->get_eventType()."\n";
            echo "Event Href: " . $auditEvent->get_href()."\n";
            echo "Org Name: " . $auditEvent->getOrg()->get_name()."\n";
        }
    }
}
...
function usage()
{
    echo "Usage:\n\n";
    echo "  [Description]\n";
    echo "     This samples can be used by System Administrator to query the Admin AuditEvents using query.\n";
    echo "\n";
    echo "  [Usage]\n";
    echo "     # php queryadminauditevents.php -s <server> -u <username> -p <password> -v <sdkversion> [Options]\n";
    echo "\n";
    ...
    echo "\n";
    echo "  [Options]\n";
    echo "     -a|--certpath <certificatepath>       [opt] Local certificate's full path.\n";
    echo "\n";
    echo "  [Examples]\n";
    echo "     # php queryadminauditevents.php -s 127.0.0.1 -u admin@Org -p password -v 5.7\n";
    echo "     # php queryadminauditevents.php -s 127.0.0.1 -u admin@Org -p password -v 5.7 -f certificatepath\n";
    echo "     # php queryadminauditevents.php // using config.php to set login credentials\n\n";
}
?>
